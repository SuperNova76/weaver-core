...
def train_classification_domain(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch=None, grad_scaler=None, tb_helper=None):
    ...
    with tqdm.tqdm(train_loader) as tq:
        for X, y_cat, _, y_domain, _, y_cat_check, y_domain_check in tq:
            ...
            label_domain = label_domain.squeeze(1)
            label_domain_check = label_domain_check.squeeze(1)            
            ...
            for idx, (k,v) in enumerate(y_domain_check.items()):
                if num_domains == 1:
                    ...
                    label_domain_np = label_domain[index_domain[k]].squeeze(1).cpu().numpy().astype(dtype=np.int32)
                    ...
                    label_domain_np = label_domain[index_domain[k],idx].squeeze(1).cpu().numpy().astype(dtype=np.int32)
                    if np.iterable(label_domain_np):
                        label_domain_counter[idx].update(label_domain_np)
                    else:
            ...
            with torch.cuda.amp.autocast(enabled=grad_scaler is not None):
                ...
                model_output_domain = model_output_domain[index_domain_all].squeeze(1).float()
                label_cat = label_cat.squeeze(0)
                if label_domain.nelement()!=1:
                    label_domain = label_domain.squeeze(0)
                if label_domain_check.nelement()!=1:
                    label_domain_check = label_domain_check.squeeze(0)
            ...
            ## multiple domain regions
                ...
                for idx, (k,v) in enumerate(y_domain_check.items()):                    
                    ...
                    label  = label_domain[index_domain[k],idx].squeeze(1)
                    if not torch.is_tensor(label) or not np.iterable(label): continue
                    pred_domain = model_output_domain[index_domain[k],id_dom:id_dom+ldomain[idx]].squeeze(1)
...
def evaluate_classification_domain(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None,
    ...
    with torch.no_grad():
        with tqdm.tqdm(test_loader) as tq:
            for X, y_cat, y_domain, Z, y_cat_check, y_domain_check in tq:
                ...
                for idx, (k, v) in enumerate(y_domain.items()):
                    if not for_training:
                        labels_domain[k].append(v.squeeze(0).cpu().numpy().astype(dtype=np.int32))
                        indexes_domain[k].append((index_offset+index_domain[list(y_domain_check.keys())[idx]]).cpu().numpy().astype(dtype=np.int32))
                    else:
                        # nonzero domain label checks
                        nz = y_domain_check[list(y_domain_check.keys())[idx]].nonzero()
                        # labels_domain[k].append(v[index_domain[list(y_domain_check.keys())[idx]]].squeeze(1).cpu().numpy().astype(dtype=np.int32))
                        labels_domain[k].append(v[nz].squeeze(1).cpu().numpy().astype(dtype=np.int32))
                ...
                if for_training:                        
                    ...
                    ### adjust outputs
                    model_output_cat = model_output_cat.squeeze(1).float()
                    model_output_domain = model_output_domain.squeeze(1).float()
                    ...
                    for idx, name in enumerate(y_domain.keys()):
                        ...
                        scores_domain[name].append(torch.softmax(score_domain[index_domain[list(y_domain_check.keys())[idx]]].squeeze(1),dim=1).cpu().numpy().astype(dtype=np.float32))
                else:
                    ...
                    for idx, name in enumerate(y_domain.keys()):
                        id_dom = idx*ldomain[idx]
                        score_domain = model_output_domain[:,id_dom:id_dom+ldomain[idx]]
                        scores_domain[name].append(torch.softmax(score_domain.squeeze(1),dim=1).cpu().numpy().astype(dtype=np.float32))
                ...
                ## multiple domains
                    ...
                    for idx, (k,v) in enumerate(y_domain_check.items()):
                        ...
                        label = label_domain[index_domain[k],idx].squeeze(1)
                        if not torch.is_tensor(label) or not np.iterable(label): continue
                        pred_domain = model_output_domain[index_domain[k],id_dom:id_dom+ldomain[id]].squeeze(1)
                        if not torch.is_tensor(pred_domain) or not np.iterable(pred_domain): continue
                        _, pred_domain = pred_domain.max(1)
                        if pred_domain.shape != label.shape: continue
                        correct_domain += (pred_domain == label).sum().item()
    ...
    if not for_training:
        indexes_cat = np.concatenate(indexes_cat).squeeze(1)
