...
Prompt_Lepton_Classifier = import_module(
    os.path.join(os.path.dirname(__file__), 'Prompt_Lepton_Classifier_domain.py'), 'prompt_lep')
...
def get_model(data_config, **kwargs):
    ...
    num_domains = []
    if hasattr(data_config, 'label_domain_value') and data_config.label_domain_value:
    ...
    cfg = dict(
        ...
        num_domains=num_domains if num_domains != [0] else [],  # Set to empty list if no domain information
        hidden_dim=kwargs.get('hidden_dim', 256),
        dropout_rate=kwargs.get('dropout_rate', 0.1),
        for_inference=kwargs.get('for_inference', False),  # Ensure for_inference is passed here
        alpha_grad=kwargs.get('alpha_grad', 0.7),  # Ensure alpha_grad is passed here
    )
...
class CrossEntropyLogCoshLossDomain(torch.nn.L1Loss):
    ...
    def forward(self, 
        ...
        ## domain terms
        loss_domain = 0
        if input_domain.nelement() and self.domain_weight:
            ## just one domain region
            if len(self.domain_weight) == 1:
...
def get_loss(data_config, **kwargs):
    ...
    ## Check if domain-related information exists
    if hasattr(data_config, 'label_domain_loss_weight') and data_config.label_domain_loss_weight:
        wdomain = data_config.label_domain_loss_weight
        ## number of labels for cross entropy in each domain
        if isinstance(data_config.label_domain_value, dict):
            ldomain = [len(dct) if isinstance(dct, list) else 1 for dct in data_config.label_domain_value.values()]
        else:
...
#             param.register_hook(print_gradients_hook)